...
```{r}
#así se define un árbol
model_tree <-
  decision_tree(cost_complexity = 0.02) %>% #el árbol sería muy grande
  set_engine("rpart") %>% #es la más usada
  set_mode("classification") %>% # se puede cambiar a regresión
  translate()
model_tree
```
...
```{r}
#tomamos el modelo y aplicamos el fit (nos interesa la supervivencia)
...
#cp: cost complexity, pone un umbral sobre la profundidad que llegará el árbol, si después de x nodo no se modifica mucho, deja de crecer
survived
```
```{r}
# all  the details
...
```
Se dividio si eras hombre o mujeres, si eres mujeres tienes un porcentaje de sobrevivir, si eres hombre tendrías que cumplir más condiciones para sobrevivir.
...
# cambiamos las ventas a un si o no, si son mayores a 8 se van a convertir en sí, y si son menores en no, agreagamos una columna nueva llamada High
...
```{r}
#siempre le tenemos que pasar un factor
...
```{r}
rpart.plot(tree.carseats$fit, type = 3, clip.right.labs = FALSE, branch = .3, under = TRUE)
...
Empezamos con la característica más importante, (los nodos es conforme a importancia)
